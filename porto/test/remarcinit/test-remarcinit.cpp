...
TEST(PortoRemarcInitTest, InitFromStorage)
{
  std::string const uri = QString("remarc-init://%1/generated.in")
    .arg(QDir::tempPath())
    .toStdString();
  auto storage = new soft::Storage("external", uri.c_str());
  ASSERT_TRUE(nullptr != storage);
  delete storage;
}

TEST(PortoRemarcInitTest, WriteAndReadFromEntity)
{
  std::string const uri = QString("remarc-init://%1/generated.in")
    .arg(QDir::tempPath())
    .toStdString();
  
  soft::Storage *storage = new soft::Storage("external", uri.c_str());
  soft::Remarcsetup *remarcConfig = new soft::Remarcsetup();
  ...
  storage = new soft::Storage("external", uri.c_str());
  remarcConfig = new soft::Remarcsetup();
